...
- **标签**：每张表情可有多个标签；主页按标签筛选（多选为 AND），另有「未标签」筛选
- **导入时打标签**：从相册/文件选图后弹出标签面板，可为整批统一打标签，也可点「稍后」跳过
- **多选批量打标签**：AppBar「多选」或长按进入选择模式 → 勾选多张 →「打标签」（为所选**追加**标签，不覆盖已有）
- **最近标签**：最近使用的标签（约 10 个）持久化；在导入 / 编辑 / 批量 / 查看页均可一键添加
- **查看页快捷标签**：全屏查看底部展示当前标签芯片（点删除），「添加」打开完整面板，并显示最近标签一键追加
- **排序**：最新优先 / 最旧优先 / 按文件名
- **添加**：从相册或文件选择；删除；编辑标签
- **分享**：主页磁贴「⋯」菜单，以及全屏查看页顶部「分享」，通过系统分享面板发送图片文件（`image/*`）
...
1. **导入打标签**：添加 → 选图 → 标签面板（已有 / 新建 / 最近）→「应用并导入」或「稍后」
2. **单张编辑**：磁贴「⋯」→ 编辑标签；或查看页底部芯片 / 顶部标签按钮
3. **批量打标签**：多选 → 勾选 →「打标签」→ 选择要追加的标签
4. **筛选未标签**：主页芯片「未标签」→ 多选 → 批量打标签，适合补标
...
| `file_picker` | 文件选择器 |
| `provider` | 简单状态管理 |
| `shared_preferences` | 网格列数、最近标签等偏好 |
...
- 最近标签名列表保存在 SharedPreferences（`recent_tag_names`）
